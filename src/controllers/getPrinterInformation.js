...
const getPrinterInformation = (() => {
    ...
    Printers.findAll({ attributes: ["nomeFila", "enderecoFila"] }).then(data => {
        ...
        data.forEach(printerData => {
            ...
            //HPs
            axios.get(urlHP).then(async response => {

                let dadosHP = getHPInfo(printerData.enderecoFila, printerData.nomeFila)

                if (response.status == "200") {
                    await printerStatusController.findByName(printerData.nomeFila).then(dadosRetorno => {
                        ...
                        console.log("Erro ao requisitar a URL HP" + error);
                    })
                }
